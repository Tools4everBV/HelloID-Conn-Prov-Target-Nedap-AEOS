####################################################################
# HelloID-Conn-Prov-Target-Nedap-AEOS-ImportPermissions-TemplateAuthorisationKeyCabinet
# PowerShell V2
####################################################################
...
try {
    Write-Information 'Starting Nedap-AEOS permission TemplateAuthorisationKeyCabinet entitlement import'
    ...
    $body = "<sch:TemplateSearchInfo><sch:TemplateInfo><sch:UnitOfAuthType>KeyCabinet</sch:UnitOfAuthType></sch:TemplateInfo></sch:TemplateSearchInfo>"
    ...
    foreach ($importedAccount in $existingAccounts) {
        ...
        foreach ($importedPermission in $responseCarrierIdProfile.Envelope.Body.ProfileResult.AuthorisationKeyCabinet.KeyTemplateAuthorisation) {
    ...
    Write-Information 'Nedap-AEOS permission TemplateAuthorisationKeyCabinet entitlement import completed'
} 
catch {
    $ex = $PSItem
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        Write-Error "Could not import Nedap-AEOS permission TemplateAuthorisationKeyCabinet entitlements. Error: $($errorObj.FriendlyMessage)"
    } 
    else {
        Write-Warning "Error at Line '$($ex.InvocationInfo.ScriptLineNumber)': $($ex.InvocationInfo.Line). Error: $($ex.Exception.Message)"
        Write-Error "Could not import Nedap-AEOS permission TemplateAuthorisationKeyCabinet entitlements. Error: $($ex.Exception.Message)"
    }
}