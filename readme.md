...
> [!IMPORTANT]
> The KeyCabinet template authorisations are drycoded and untested.
...
- **HelloID on-premise agent**: Connection to the SOAP web service is mandatory. Since AEOS is usually hosted on-premise, the HelloID on-premise agent is needed. 
...
> Free fields are supported, it should be mapped including the definitionId. Example: Freefield12345. To find out what the definitionId's are, use the script in folder 'supportscripts'
...
- **Disable Account**: Sets the `LeaveDateTime` to the current time.
- **Delete Account**: Sets the `LeaveDateTime` to the current time and withdraws all carrier tokens (badges) from the employee.
- Badges are only removed from the account when deleting. Creating and assigning badges are not in scope of this connector
...
| DepartmentSearchInfo   | Search for all departments                       |
